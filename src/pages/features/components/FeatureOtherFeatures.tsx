...
export default function FeatureOtherFeatures({ currentSlug }: FeatureOtherFeaturesProps) {
  const others = featureDetails.filter((f) => f.slug !== currentSlug).slice(0, 3);
        ...
        <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-3 gap-5 md:gap-6">
          {others.map((f, i) => (
            ...
            >
              ...
              <p className="text-sm text-gray-600 leading-relaxed flex-1">{f.tagline}</p>
