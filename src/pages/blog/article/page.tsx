...
const SKELETON_WIDTHS = ['92%', '100%', '88%', '95%', '83%', '97%', '90%', '86%'];
...
async function getArticleBody(category: string, id?: string): Promise<{ paragraphs: string[]; takeaways: string[] }> {
  const { articleBodyById, articleBodyByCategory, articleTakeawaysById } = await import('../../../mocks/articleBodies');
  const paragraphs = (id && articleBodyById[id]) ? articleBodyById[id] : (articleBodyByCategory[category] ?? articleBodyByCategory['Best Practices']);
  const takeaways = (id && articleTakeawaysById[id]) ? articleTakeawaysById[id] : [
...
const ShareBar = memo(function ShareBar({ articleId, title }: { articleId: string; title: string }) {
  return (
    <div className="mt-8 flex items-center gap-3">
      ...
      <a
        ...
        onClick={() => track('article_shared', { article_id: articleId, channel: 'twitter' })}
      ...
      <a
        ...
        target="_blank"
        rel="nofollow noopener noreferrer"
        onClick={() => track('article_shared', { article_id: articleId, channel: 'linkedin' })}
        className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-teal-50 hover:text-teal-600 text-gray-500 transition-colors cursor-pointer"
      >
      ...
      <button
        onClick={() => {
          ...
          track('article_shared', { article_id: articleId, channel: 'copy_link' });
        }}
        className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-teal-50 hover:text-teal-600 text-gray-500 transition-colors cursor-pointer"
        title="Copy link"
      >
...
export default function ArticlePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setBodyLoading(true);
    getArticleBody(article.category, article.id).then(({ paragraphs, takeaways: t }) => {
      if (!cancelled) {
  ...
  const seoData = useMemo(() => {
    if (!article)
      return {
        ...
        geoData: null as null | { region: string; position: string; placename: string },
      };

    const iso = (() => {
      try { return new Date(article.date).toISOString(); }
      catch { return new Date().toISOString(); }
    })();

    const detectedCountry =
      detectCountryFromText(article.title) ?? detectCountryFromText(article.excerpt);
    const geoData = detectedCountry
      ? { region: detectedCountry.code, position: detectedCountry.position, placename: detectedCountry.name }
      : null;
    ...
    const schema: object[] = [
      {
        ...
        wordCount: bodyParagraphs.length > 0 ? bodyParagraphs.join(' ').split(' ').length : 800,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: article.title, item: `${SITE_URL}/blog/${article.id}` },
        ],
      },
    ];
      ...
      <article className="max-w-4xl mx-auto px-6 lg:px-8 py-12">
        ...
        <div className="prose prose-lg max-w-none">
          {bodyLoading ? (
            ...
            <div className="space-y-4 animate-pulse" aria-busy="true" aria-label="Loading article content">
        ...
        <div className="mt-10 p-6 bg-gray-50 rounded-xl border border-gray-100 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-teal-600 mb-2">{article.authorRole} · VerifyAfrica</p>
            <p className="text-sm text-gray-500 leading-relaxed">
              A compliance and regulatory expert at VerifyAfrica with deep experience across
              African financial markets, helping organisations build scalable KYC and AML
              programmes.
            </p>
          </div>
        </div>

        <ShareBar articleId={article.id} title={article.title} />
      </article>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {displayRelated.map((rel) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <span className="text-xs font-semibold text-teal-600">{rel.category}</span>
                    <h3 className="text-sm font-bold text-gray-900 mt-1 line-clamp-2 group-hover:text-teal-700 transition-colors leading-snug">
                      {rel.title}
                    </h3>
      ...
      >
        <div className="max-w-4xl mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-500 text-sm mb-6 max-w-xl mx-auto">
            See how VerifyAfrica&apos;s AI-powered platform can automate your KYC, AML, and identity
            verification workflows across all 54 African markets.
          </p>
