...
export default function UseCaseModal({ useCase, isOpen, onClose, onRequestDemo }: UseCaseModalProps) {
  useEffect(() => {
    return () => {
      ...
      <div className="relative bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="relative h-56 sm:h-64">
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-end gap-4">
            <div className={`w-14 h-14 flex items-center justify-center rounded-xl bg-gradient-to-br ${useCase.color} shadow-lg`}>
              <i className={`${useCase.icon} text-2xl text-white`}></i>
            </div>
            <div>
              <h2 className="text-2xl sm:text-3xl font-bold text-white">{useCase.category}</h2>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8 p-5 bg-gradient-to-br from-teal-50 to-cyan-50 rounded-xl">
            {useCase.benefits.map((benefit, index) => (
              <div key={index} className="text-center">
                <div className="text-2xl sm:text-3xl font-bold text-teal-600">{benefit.metric}</div>
          ...
          <div className="mb-8 p-5 bg-gray-50 rounded-xl">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2 text-gray-600 text-sm">
                ...
                <span>Pan-African coverage with 35+ countries and 50+ ID types</span>
              </li>
              <li className="flex items-start gap-2 text-gray-600 text-sm">
                <i className="ri-arrow-right-s-line text-teal-500 mt-0.5"></i>
              ...
              <li className="flex items-start gap-2 text-gray-600 text-sm">
                <i className="ri-arrow-right-s-line text-teal-500 mt-0.5"></i>
                <span>Full regulatory compliance across all African markets</span>
              </li>
              <li className="flex items-start gap-2 text-gray-600 text-sm">
                <i className="ri-arrow-right-s-line text-teal-500 mt-0.5"></i>
                <span>Easy API integration with comprehensive documentation</span>
              </li>
            </ul>
          </div>
