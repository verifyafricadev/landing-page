...
const regionZoomConfig: Record<string, { x: number; y: number; scale: number }> = {
...
const regionBounds: Record<string, { x: number; y: number; width: number; height: number }> = {
...
export default function AfricaMapVisualization() {
  ...
  const searchResults = searchQuery.trim()
    ? countriesServed.filter(country =>
  ...
  const regionCounts = {
    'West Africa': countriesServed.filter(c => c.region === 'West Africa').length,
    'East Africa': countriesServed.filter(c => c.region === 'East Africa').length,
    'Southern Africa': countriesServed.filter(c => c.region === 'Southern Africa').length,
    'North Africa': countriesServed.filter(c => c.region === 'North Africa').length,
    'Central Africa': countriesServed.filter(c => c.region === 'Central Africa').length
  };

  // Calculate zoom transform
  const getZoomTransform = () => {
    if (!selectedRegion) return { transform: 'scale(1) translate(0, 0)', transformOrigin: 'center center' };
    
    const config = regionZoomConfig[selectedRegion];
    if (!config) return { transform: 'scale(1) translate(0, 0)', transformOrigin: 'center center' };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-teal-50 border border-teal-100 rounded-full mb-4">
            ...
            <span className="text-teal-700 text-sm font-medium">Pan-African Coverage</span>
          </div>
          <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">
            Coverage Across Africa
          </h2>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto mb-8">
            Interactive map showing our identity verification coverage across all 54 African countries
          </p>
          ...
          <div ref={searchRef} className="relative max-w-md mx-auto">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-3 bg-white border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent shadow-sm transition-all"
              />
              {searchQuery && (
                <button
                  onClick={clearSearch}
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-xl z-50 max-h-72 overflow-y-auto">
                {searchResults.length > 0 ? (
                  <div className="py-2">
                    {searchResults.map((country) => (
                      ...
                      >
                        ...
                        <div className="text-xs text-gray-400 font-mono">{country.code}</div>
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="px-4 py-6 text-center">
                    ...
                    <p className="text-sm text-gray-500">No countries found for "{searchQuery}"</p>
                  </div>
                )}
              </div>
            )}
          ...
          {highlightedCountry && (
            <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-teal-500 text-white rounded-full animate-pulse">
              ...
              <span className="text-sm font-medium">
                Showing: {countriesServed.find(c => c.code === highlightedCountry)?.name}
              </span>
              <button onClick={clearSearch} className="ml-1 hover:bg-teal-600 rounded-full p-0.5 cursor-pointer">
          ...
          {/* Zoomed Region Badge */}
          {selectedRegion && (
            <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-gray-900 text-white rounded-full">
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-xl transition-all cursor-pointer ${
                      selectedRegion === region
                        ? 'bg-gray-900 text-white ring-2 ring-teal-500 ring-offset-2'
            ...
            {hoveredCountry && (
              <div className="mt-4 bg-white rounded-2xl shadow-lg border border-gray-100 p-6 animate-fade-in">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  />
                  <div>
                    <h4 className="font-semibold text-gray-900">{hoveredCountry.name}</h4>
                    <p className="text-xs text-gray-500">{hoveredCountry.region}</p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 order-1 lg:order-2">
            ...
            >
              ...
              {selectedRegion && (
                ...
                >
                  ...
                  <div className="text-center mt-1 flex items-center justify-center gap-1">
                    ...
                    <span className="text-[10px] text-white/80 font-medium">You are here</span>
                  </div>
                </div>
              )}
              ...
              >
                ...
                {(selectedRegion ? filteredCountries : countriesServed).map((country) => {
                  const position = countryPositions[country.code];
                  if (!position) return null;

                  const isHovered = hoveredCountry?.code === country.code;
                  const isHighlighted = highlightedCountry === country.code;
                  const isInSelectedRegion = selectedRegion === country.region;
                    ...
                    >
                      ...
                      {(isHovered || isHighlighted) && (
                        <div
                          ...
                          style={{
                            backgroundColor: isHighlighted ? '#14b8a6' : color,
                      ...
                      {(isHovered || isHighlighted) && (
                        <div className="absolute left-1/2 -translate-x-1/2 -top-12 whitespace-nowrap">
                          <div className={`px-3 py-1.5 rounded-full shadow-lg text-xs font-semibold flex items-center gap-2 ${
                            isHighlighted ? 'bg-teal-500 text-white' : 'bg-white text-gray-900'
                          }`}>
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                <div className={`text-white/60 text-xs ${selectedRegion ? 'ml-32' : ''}`}>
                  {selectedRegion ? `Viewing ${filteredCountries.length} countries in ${selectedRegion}` : 'Click a region to zoom in'}
                </div>
