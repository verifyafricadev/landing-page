...
const countryDetails: Record<string, { population?: string; capital?: string; currency?: string; languages?: string[]; verificationPartners?: string[] }> = {
  'NG': { population: '223M+', capital: 'Abuja', currency: 'Nigerian Naira (NGN)', languages: ['English', 'Hausa', 'Yoruba', 'Igbo'], verificationPartners: ['NIMC', 'NIBSS', 'FRSC'] },
  'GH': { population: '33M+', capital: 'Accra', currency: 'Ghanaian Cedi (GHS)', languages: ['English', 'Akan', 'Ewe'], verificationPartners: ['NIA', 'DVLA', 'EC'] },
  'KE': { population: '54M+', capital: 'Nairobi', currency: 'Kenyan Shilling (KES)', languages: ['English', 'Swahili'], verificationPartners: ['IPRS', 'NTSA', 'IEBC'] },
  'ZA': { population: '60M+', capital: 'Pretoria', currency: 'South African Rand (ZAR)', languages: ['English', 'Zulu', 'Xhosa', 'Afrikaans'], verificationPartners: ['DHA', 'SARS'] },
  'EG': { population: '104M+', capital: 'Cairo', currency: 'Egyptian Pound (EGP)', languages: ['Arabic'], verificationPartners: ['Civil Registry', 'MOI'] },
  'ET': { population: '120M+', capital: 'Addis Ababa', currency: 'Ethiopian Birr (ETB)', languages: ['Amharic', 'Oromo', 'Tigrinya'], verificationPartners: ['NIDP', 'INSA'] },
  'TZ': { population: '65M+', capital: 'Dodoma', currency: 'Tanzanian Shilling (TZS)', languages: ['Swahili', 'English'], verificationPartners: ['NIDA', 'TRA'] },
  'MA': { population: '37M+', capital: 'Rabat', currency: 'Moroccan Dirham (MAD)', languages: ['Arabic', 'Berber', 'French'], verificationPartners: ['DGSN', 'CNIE'] },
  'SN': { population: '17M+', capital: 'Dakar', currency: 'West African CFA Franc (XOF)', languages: ['French', 'Wolof'], verificationPartners: ['DAF', 'DGID'] },
  'CI': { population: '28M+', capital: 'Yamoussoukro', currency: 'West African CFA Franc (XOF)', languages: ['French'], verificationPartners: ['ONI', 'DGTTC'] },
  'CM': { population: '28M+', capital: 'Yaoundé', currency: 'Central African CFA Franc (XAF)', languages: ['French', 'English'], verificationPartners: ['BUNEC', 'DGSN'] },
  'UG': { population: '47M+', capital: 'Kampala', currency: 'Ugandan Shilling (UGX)', languages: ['English', 'Swahili'], verificationPartners: ['NIRA', 'URA'] },
  'RW': { population: '14M+', capital: 'Kigali', currency: 'Rwandan Franc (RWF)', languages: ['Kinyarwanda', 'English', 'French'], verificationPartners: ['NIDA', 'RRA'] },
  'ZM': { population: '20M+', capital: 'Lusaka', currency: 'Zambian Kwacha (ZMW)', languages: ['English', 'Bemba', 'Nyanja'], verificationPartners: ['DNRPC', 'RTSA'] },
  'ZW': { population: '16M+', capital: 'Harare', currency: 'Zimbabwe Dollar (ZWL)', languages: ['English', 'Shona', 'Ndebele'], verificationPartners: ['RG Office', 'ZIMRA'] },
};
...
export default function CountryDetailModal({ country, onClose }: CountryDetailModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header with Flag */}
        <div className={`relative bg-gradient-to-r ${regionGradient} p-8 pb-20`}>
          {/* Close Button */}
          <button
            onClick={onClose}
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/20 rounded-full mb-4">
            ...
            <span className="text-white text-sm font-medium">{country.region}</span>
          </div>
        ...
        <div className="p-8 pt-12 overflow-y-auto max-h-[50vh]">
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {details.population && (
              <div className="text-center p-4 bg-gray-50 rounded-xl">
                ...
                <div className="text-lg font-bold text-gray-900">{details.population}</div>
            ...
            {details.capital && (
              <div className="text-center p-4 bg-gray-50 rounded-xl">
                ...
                <div className="text-lg font-bold text-gray-900">{details.capital}</div>
            ...
            {details.currency && (
              <div className="text-center p-4 bg-gray-50 rounded-xl">
                ...
                <div className="text-sm font-bold text-gray-900">{details.currency.split(' ')[0]}</div>
            ...
            {!details.population && !details.capital && !details.currency && (
              <>
                ...
                <div className="text-center p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="text-lg font-bold text-gray-900">&lt;3s</div>
          ...
          <div className="mb-8">
            <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <i className="ri-id-card-line" style={{ color: regionColor }}></i>
              Supported ID Types
            </h3>
          ...
          {details.languages && details.languages.length > 0 && (
            <div className="mb-8">
              <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <i className="ri-translate-2" style={{ color: regionColor }}></i>
                Languages Supported
              </h3>
              <div className="flex flex-wrap gap-2">
          ...
          {details.verificationPartners && details.verificationPartners.length > 0 && (
            <div className="mb-8">
              <h3 className="text-sm font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <i className="ri-links-line" style={{ color: regionColor }}></i>
                Verification Partners
              </h3>
              <div className="flex flex-wrap gap-2">
          ...
          <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-6">
            <h3 className="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <i className="ri-shield-star-line" style={{ color: regionColor }}></i>
              Verification Features
            </h3>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="px-6 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-full hover:bg-gray-800 transition-colors cursor-pointer whitespace-nowrap"
            >
              Close
            </button>
          </div>
