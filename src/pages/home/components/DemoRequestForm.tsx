...
export default function DemoRequestForm({ isOpen, onClose }: DemoRequestFormProps) {
  ...
  const validate = (form: HTMLFormElement) => {
    ...
    if (!data.get('first_name')) newErrors.first_name = 'First name is required';
    ...
    const blockedDomains = [
      ...
      'gmx.com', 'gmx.net', 'gmx.de', 'web.de', 'inbox.com',
      'fastmail.com', 'hushmail.com', 'rediffmail.com', 'lycos.com',
      'rocketmail.com', 'att.net', 'verizon.net', 'comcast.net', 'sbcglobal.net',
    ];
    if (!emailVal || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailVal)) {
      ...
      if (blockedDomains.includes(domain)) {
        newErrors.email = 'Please use your company email address (personal emails like Gmail or Yahoo are not accepted)';
      }
    }
    if (!data.get('company')) newErrors.company = 'Company name is required';
    if (!data.get('company_size')) newErrors.company_size = 'Please select company size';
    const message = data.get('message') as string;
    if (message && message.length > 500) newErrors.message = 'Message must be under 500 characters';
    return newErrors;
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const subject = encodeURIComponent(`Demo Request from ${firstName} ${lastName} - ${company}`);
...
This demo request was submitted via the VerifyAfrica website.`
  ...
  const inputBase = 'w-full px-4 py-3 text-sm bg-white border rounded-md outline-none transition-all duration-200 focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 placeholder:text-gray-400';
            ...
            <>
              <div className="mb-6">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-teal-100 text-teal-700 text-xs font-semibold tracking-wide uppercase mb-4">
                  <span className="w-4 h-4 flex items-center justify-center mr-1.5"><i className="ri-calendar-line text-sm"></i></span>
                  Schedule a Demo
                </div>
                <h3 className="text-2xl font-bold text-gray-900 mb-2">See VerifyAfrica in Action</h3>
                <p className="text-sm text-gray-500">Get a personalized walkthrough of our identity verification platform.</p>
              </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="company_size" className={labelBase}>Company Size *</label>
                    <select
                      id="company_size"
                      name="company_size"
                      defaultValue=""
                    ...
                    {errors.company_size && <p className="text-xs text-red-500 mt-1">{errors.company_size}</p>}
                  </div>
                  <div>
                ...
                {/* Message */}
                <div>
                  <label htmlFor="message" className={labelBase}>What are you looking to verify?</label>
                  <textarea
                    id="message"
                    name="message"
                    rows={3}
                ...
                <p className="text-xs text-gray-400 text-center flex items-center justify-center gap-1.5">
                  <span className="w-3.5 h-3.5 flex items-center justify-center"><i className="ri-lock-line text-xs"></i></span>
