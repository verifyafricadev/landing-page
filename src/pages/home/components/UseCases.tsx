...
export default function UseCases() {
	...
	const useCases = [
		...
		},
		{
			...
			title: "Banks",
			description:
				"End-to-end customer due diligence and regulatory compliance for banking institutions.",
		...
		},
		{
			...
			title: "FX & Brokers",
			description:
				"Risk-based onboarding and monitoring for cross-border clients.",
	...
	return (
		...
		>
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
				...
				<div
					ref={gridRef}
					className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5 sm:gap-4 lg:gap-6"
				>
					{useCases.map((useCase, index) => (
						...
						>
							...
							<div
								className={`absolute inset-0 bg-gradient-to-br ${useCase.hoverGradient} opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
							></div>

							<div className="relative z-10">
								<div className="w-8 h-8 sm:w-11 lg:w-14 sm:h-11 lg:h-14 flex items-center justify-center bg-white/20 backdrop-blur-sm rounded-md sm:rounded-lg mb-2 sm:mb-4 lg:mb-6 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 group-hover:bg-white/30">
									<i
										className={`${useCase.icon} text-lg sm:text-xl lg:text-3xl transition-transform duration-300 group-hover:scale-110`}
									></i>
								</div>
								<h3 className="text-xs sm:text-sm lg:text-xl font-bold mb-0.5 sm:mb-2 lg:mb-3 transition-transform duration-300 group-hover:translate-x-1">
									{useCase.title}
								</h3>
								<p className="text-[10px] sm:text-xs lg:text-sm text-white/90 leading-relaxed hidden sm:block">
									{useCase.description}
								</p>
							</div>
							...
							<div
								className="absolute bottom-8 right-8 w-1.5 h-1.5 bg-white/20 rounded-full opacity-0 group-hover:opacity-100 group-hover:animate-float-delayed transition-opacity duration-300 hidden sm:block"
