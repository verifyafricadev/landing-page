...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-12 sm:py-16 lg:py-24 bg-gray-50 overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-12">
        ...
        >
          ...
          <p className="text-sm sm:text-base lg:text-xl text-gray-600 px-2">Everything you need to know about VerifyAfrica</p>
        </div>

        <div ref={listRef} className="space-y-2.5 sm:space-y-4">
          {faqs.map((faq, index) => (
            <div
              key={index}
            ...
            >
              ...
              >
                <span className={`text-xs sm:text-sm md:text-base lg:text-lg font-semibold pr-3 sm:pr-6 lg:pr-8 transition-colors duration-300 ${
                  openIndex === index ? 'text-teal-700' : 'text-gray-900'
                }`}>
                  {faq.question}
                </span>
                <div className={`w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-full transition-all duration-300 flex-shrink-0 ${
                  openIndex === index 
                    ? 'bg-teal-100 rotate-180' 
                    : 'bg-gray-100 group-hover:bg-teal-50'
                }`}>
                  <i className={`ri-arrow-down-s-line text-base sm:text-xl transition-colors duration-300 ${
              ...
              >
                <div className="overflow-hidden">
                  <div className={`px-4 sm:px-6 lg:px-8 pb-3.5 sm:pb-5 lg:pb-6 transition-all duration-300 ${
