...
type CountriesSection = { type: 'countries'; label: string; countries: CountryGroup[] };
    ...
    countries: [
      {
        ...
        items: [{ id: 'za_said_verification', name: 'South Africa ID Verification' }],
      },
      {
        flag: '🇳🇬',
        country: 'Nigeria',
        items: [
          ...
          { id: 'ng_advanced_phone_number_verification', name: 'Nigeria Phone Verification' },
          { id: 'ng_phone_number_lookup', name: 'Nigeria Phone Lookup' },
          { id: 'ng_cac_lookup', name: 'Nigeria CAC Lookup' },
          { id: 'ng_passport_verification', name: 'Nigeria Passport Verification' },
        ],
      },
      {
        flag: '🇬🇭',
...
export default function DocsSidebar({
  ...
  const filteredIds = query
    ? new Set(allItems.filter((i) => i.name.toLowerCase().includes(query)).map((i) => i.id))
    : null;
                  ...
                  <div className="ml-2 mt-1 space-y-0.5">
                    {section.countries.map((cg) => {
                      const visibleItems = filteredIds
                        ...
                        : cg.items;
                      if (visibleItems.length === 0) return null;
                      const isCountryCollapsed = filteredIds ? false : collapsedCountries[cg.country];
                      return (
                        <div key={cg.country}>
                          <button
                          ...
                          >
                            <span>
                              {cg.flag} {cg.country}
                              <span className="ml-1 text-xs text-gray-400">({cg.items.length})</span>
                            </span>
                            <i
