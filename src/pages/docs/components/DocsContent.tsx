...
const LINK_PARAMS: ParamSpec[] = [
	...
	},
	{
		...
		type: "object",
		required: false,
		description: "Document verification configuration for link mode.",
		nested: [
			{
				name: "backside_proof_required",
				type: "string",
				required: false,
				description:
					'"1" to require a back image, "0" to skip it. Defaults to "0".',
			},
			{
				name: "allow_online",
				type: "string",
				required: false,
				description:
					'"1" to allow online capture, "0" to disable. Defaults to "1".',
			},
			{
				name: "allow_offline",
				type: "string",
				required: false,
				description:
					'"1" to allow offline upload, "0" to disable. Defaults to "1".',
			},
		],
	},
];

const LINK_SAMPLE_INPUT = {
	language: "EN",
...
const LINK_RESPONSE = {
	...
	data: {
		...
		verification_url: "https://verify.verifyafrica.com/verification/abc123",
	},
};
...
const configs: Record<string, VerifConfig> = {
	id_document: {
		...
		isLinkMode: true,
		params: LINK_PARAMS,
		sampleInputData: LINK_SAMPLE_INPUT,
		linkSampleResponse: LINK_RESPONSE,
	},
	face_match: {
		...
		linkSampleResponse: {
			...LINK_RESPONSE,
			data: { ...LINK_RESPONSE.data, verification_type: "face_match" },
		},
	},
	aml_screening: {
		name: "AML Screening",
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "Subject's nationality.",
			},
		],
		sampleInputData: {
			first_name: "John",
		...
		linkSampleResponse: {
			...
			data: {
				...
				response_data: { match_found: false, sanctions: [], pep: false },
			},
		},
	},
	business_aml_screening: {
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "Registration or company number.",
			},
		],
		sampleInputData: {
		...
		linkSampleResponse: {
			...
			data: {
				...
				status: "clear",
				created_at: "2026-01-15T10:30:00Z",
				response_data: { match_found: false, sanctions: [] },
			},
		},
	},
	kyb_screening: {
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "Legal name of the business (used for cross-validation).",
			},
		],
		sampleInputData: {
			rc_number: "RC789012",
			company_name: "TechStart Nigeria Ltd",
		},
		linkSampleResponse: {
			status: "success",
			data: {
	...
	address_verification: {
		...
		linkSampleResponse: {
			...
			data: {
				...
				status: "verified",
				created_at: "2026-01-15T10:30:00Z",
				response_data: { address_confirmed: true, confidence_score: 0.97 },
			},
		},
	},
	risk_assessment: {
		name: "Risk Assessment",
		verificationType: "risk_assessment",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "The identity number to assess.",
			},
			{
				name: "id_type",
				type: "string",
				required: true,
				description: 'Type of ID (e.g., "national_id", "passport", "bvn").',
			},
			{
				name: "country",
				type: "string",
		...
		linkSampleResponse: {
			...
			data: {
				...
				status: "completed",
				created_at: "2026-01-15T10:30:00Z",
				response_data: { risk_score: 12, risk_level: "low", flags: [] },
			},
		},
	},
	crypto_wallet_screening: {
		name: "Crypto Wallet Screening",
		verificationType: "crypto_wallet_screening",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "The cryptocurrency wallet address to screen.",
			},
			{
				name: "blockchain",
				type: "string",
				required: true,
				description:
					'Blockchain network (e.g., "bitcoin", "ethereum", "tron").',
			},
		],
		sampleInputData: {
	...
	za_said_verification: {
		...
		verificationType: "za_said_verification",
		description:
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "Subject's first name for cross-validation.",
			},
			{
				name: "last_name",
				type: "string",
				required: false,
				description: "Subject's last name for cross-validation.",
			},
		],
		sampleInputData: {
	...
	ng_bvn_verification: {
		...
		verificationType: "ng_bvn_verification",
		description:
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "First name for cross-validation.",
			},
			{
				name: "last_name",
				type: "string",
				required: false,
				description: "Last name for cross-validation.",
			},
			{
				name: "date_of_birth",
				type: "string",
	...
	ng_nin_verification: {
		...
		verificationType: "ng_nin_verification",
		description:
		...
		sampleInputData: { nin: "12345678901" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ng_virtual_nin_verification: {
		name: "Nigeria Virtual NIN",
		verificationType: "ng_virtual_nin_verification",
		description:
		...
		sampleInputData: { vnin: "AB123456789012CD" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ng_advanced_phone_number_verification: {
		...
		sampleInputData: { phone_number: "08012345678" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ng_phone_number_lookup: {
		name: "Nigeria Phone Lookup",
		verificationType: "ng_phone_number_lookup",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Nigerian phone number to look up.",
			},
		],
		sampleInputData: { phone_number: "08012345678" },
		linkSampleResponse: {
	...
	ng_cac_lookup: {
		name: "Nigeria CAC Lookup",
		verificationType: "ng_cac_lookup",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "CAC registration number (e.g., RC123456).",
			},
			{
				name: "company_name",
				type: "string",
				required: false,
				description: "Company name for cross-validation.",
			},
		],
		sampleInputData: { rc_number: "RC123456" },
		linkSampleResponse: {
			status: "success",
			data: {
				id: "ver_cac123",
	...
	ng_passport_verification: {
		...
		verificationType: "ng_passport_verification",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Passport number (e.g., A12345678).",
			},
			{
				name: "first_name",
				type: "string",
		...
		sampleInputData: { passport_number: "A12345678" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	gh_passport_lookup: {
		name: "Ghana Passport Lookup",
		verificationType: "gh_passport_lookup",
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Ghana passport number (e.g., G1234567).",
			},
			{
				name: "first_name",
				type: "string",
		...
		sampleInputData: { passport_number: "G1234567" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	gh_voter_card_lookup: {
		name: "Ghana Voter Card Lookup",
		verificationType: "gh_voter_card_lookup",
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Ghana voter ID number.",
			},
			{
				name: "first_name",
				type: "string",
		...
		sampleInputData: { voter_id: "1234567890" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	gh_ssnit_lookup: {
		name: "Ghana SSNIT Lookup",
		verificationType: "gh_ssnit_lookup",
		description:
		...
		sampleInputData: { ssnit_number: "C123456789012" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: { ...DIRECT_RESPONSE.data, verification_type: "gh_ssnit_lookup" },
		},
	},
	gh_drivers_license_lookup: {
		name: "Ghana Driver's License",
		verificationType: "gh_drivers_license_lookup",
		description:
		...
		sampleInputData: { license_number: "DL123456789" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ke_passport_lookup: {
		name: "Kenya Passport Lookup",
		verificationType: "ke_passport_lookup",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Kenya passport number (e.g., A1234567).",
			},
			{
				name: "first_name",
				type: "string",
		...
		sampleInputData: { passport_number: "A1234567" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ke_national_id_lookup: {
		name: "Kenya National ID Lookup",
		verificationType: "ke_national_id_lookup",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Kenya national ID number.",
			},
			{
				name: "first_name",
				type: "string",
		...
		sampleInputData: { id_number: "12345678" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ke_phone_number_lookup: {
		name: "Kenya Phone Lookup",
		verificationType: "ke_phone_number_lookup",
		description:
		...
		params: [
			{
				...
				type: "string",
				required: true,
				description: "Kenyan phone number (e.g., 0712345678).",
			},
		],
		sampleInputData: { phone_number: "0712345678" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
	...
	ke_tax_pin_verification: {
		name: "Kenya Tax PIN Verification",
		verificationType: "ke_tax_pin_verification",
		...
		params: [
			...
			},
			{
				...
				type: "string",
				required: false,
				description: "Taxpayer name for cross-validation.",
			},
		],
		sampleInputData: { tax_pin: "A123456789B" },
		linkSampleResponse: {
			...DIRECT_RESPONSE,
			data: {
				...DIRECT_RESPONSE.data,
...
const RESPONSE_CODES = [
	...
	{
		code: "403",
		desc: "Forbidden - Insufficient credits or inactive key",
		isError: false,
	},
	{
		code: "404",
		desc: "Not Found - Identity record not found",
		isError: false,
	},
	{
		code: "500",
		desc: "Internal Server Error - Please try again",
		isError: true,
	},
];
...
function ParamRow({ param, depth = 0 }: { param: ParamSpec; depth?: number }) {
	return (
		<>
			<div
				className={`pb-4 border-b border-gray-100 last:border-0 last:pb-0 ${depth > 0 ? "ml-4 pl-4 border-l border-gray-200" : ""}`}
			>
				<div className="flex items-center flex-wrap gap-2 mb-1">
					<span className="font-mono text-sm text-secondary">{param.name}</span>
					<span className="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">
						{param.type}
					</span>
					<span
						className={`text-xs px-1.5 py-0.5 rounded ${param.required ? "text-red-600 bg-red-50" : "text-gray-500 bg-gray-100"}`}
					>
...
const BULK_VERIFICATION_TYPES = [
	...
	{
		id: "ng_virtual_nin_verification",
		name: "Nigeria Virtual NIN",
		params: [{ name: "vnin", required: true }],
	},
	{
	...
	{
		id: "gh_voter_card_lookup",
		name: "Ghana Voter Card Lookup",
		params: [
			{ name: "voter_id", required: true },
			{ name: "first_name", required: false },
		],
	},
	{
		...
		name: "Ghana SSNIT Lookup",
		params: [
			{ name: "ssnit_number", required: true },
			{ name: "first_name", required: false },
		],
	},
	{
		...
		name: "Ghana Driver's License",
		params: [
			{ name: "license_number", required: true },
			{ name: "first_name", required: false },
		],
	},
	{
		id: "ke_passport_lookup",
		name: "Kenya Passport Lookup",
		params: [
			{ name: "passport_number", required: true },
			{ name: "first_name", required: false },
	...
	{
		id: "ke_phone_number_lookup",
		name: "Kenya Phone Lookup",
		params: [{ name: "phone_number", required: true }],
	},
	{
		id: "ke_tax_pin_verification",
		name: "Kenya Tax PIN Verification",
		params: [
			{ name: "tax_pin", required: true },
...
const BULK_RESPONSE_CODES = [
	{
		code: "200",
		desc: "Success - Batch submitted and all results returned",
		isError: false,
	},
	{
		code: "207",
		desc: "Multi-Status - Batch submitted; some records failed",
		isError: false,
	},
	{
		code: "400",
		desc: "Bad Request - Missing or invalid batch parameters",
		isError: false,
	},
	{
		code: "401",
	...
	{
		code: "403",
		desc: "Forbidden - Insufficient credits for batch size",
		isError: false,
	},
	{
		code: "422",
		desc: "Unprocessable Entity - Batch size exceeds 100 records",
		isError: false,
	},
	...
	return (
		<div className="flex-1 min-w-0">
			<div className="bg-white rounded-xl border border-gray-200 p-6 lg:p-8 space-y-8">
				{/* Header */}
				<div>
					<h2 className="text-2xl lg:text-3xl font-bold text-secondary mb-1">
						Bulk Verification
					</h2>
					...
					<p className="text-gray-600">
						Submit multiple identity verification requests in a single API call.
						Bulk verification is ideal for batch onboarding, periodic compliance
						checks, and large-scale KYC workflows. You can mix different
						verification types in one batch — up to <strong>100 records</strong>{" "}
						per request.
					</p>
				</div>
				...
				<div>
					...
					<div className="flex items-center gap-3 bg-gray-50 rounded-lg px-4 py-3 border border-gray-200">
						<span className="px-2.5 py-1 bg-green-100 text-green-700 text-xs font-bold rounded whitespace-nowrap">
							POST
						</span>
					...
					<div className="bg-gray-50 rounded-lg border border-gray-200 p-4 space-y-4">
						<div className="pb-4 border-b border-gray-100">
							<div className="flex items-center flex-wrap gap-2 mb-1">
								...
								<span className="text-xs px-1.5 py-0.5 rounded text-gray-500 bg-gray-100">
									optional
								</span>
							</div>
							<p className="text-sm text-gray-600">
								...
								to run in test/sandbox mode. Defaults to{" "}
								<code className="font-mono text-xs bg-gray-100 px-1 rounded">
									false
								</code>
								.
						...
						<div>
							<div className="flex items-center flex-wrap gap-2 mb-1">
								...
								<span className="text-xs px-1.5 py-0.5 rounded text-red-600 bg-red-50">
									required
								</span>
							</div>
							<p className="text-sm text-gray-600 mb-3">
								Array of verification objects. Maximum <strong>100</strong>{" "}
								items per request. Each object must include:
							</p>
							<div className="ml-4 pl-4 border-l border-gray-200 space-y-3">
								<div>
									...
									<p className="text-sm text-gray-600">
										The type of verification to perform (see supported types
										below).
									</p>
								</div>
								<div>
									...
									<p className="text-sm text-gray-600">
										Verification-specific data fields. See the supported types
										table below for per-type parameters.
									</p>
								</div>
							</div>
						</div>
				...
				<div>
					...
					<p className="text-sm text-gray-600 mb-4">
						Note: Identity Verification types that require a hosted page
						(Document Verification, Facial Screening) are not supported in bulk
						mode. All government registry checks and compliance checks are
						supported.
					</p>
								...
								{BULK_VERIFICATION_TYPES.map((vt) => (
									<>
										<tr
											...
											onClick={() =>
												setExpandedType(expandedType === vt.id ? null : vt.id)
											}
										>
											<td className="px-4 py-3">
												<code className="text-xs font-mono text-teal-700 bg-teal-50 px-1.5 py-0.5 rounded">
													{vt.id}
												</code>
											</td>
												...
												>
													...
													<div className="flex flex-wrap gap-2">
														{vt.params.map((p) => (
															<span
																key={p.name}
																className={`text-xs px-2 py-1 rounded-full font-mono ${p.required ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-600"}`}
															>
																{p.name}{" "}
					...
					<div className="flex flex-wrap gap-1 border-b border-gray-200 pb-3 mb-3">
						{BULK_CODE_LANGS.map((lang) => (
							<button
								...
								className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all whitespace-nowrap cursor-pointer ${selectedLang === lang ? "bg-secondary text-white" : "text-gray-600 hover:bg-gray-100"}`}
							>
								{lang}
							</button>
						))}
				...
				<div>
					...
					<pre className="bg-teal-900 rounded-lg p-4 overflow-x-auto">
						<code className="text-sm text-teal-100 whitespace-pre">
							{responseJson}
						</code>
					</pre>
				</div>
						...
						{[
							{
								name: "batch_id",
								type: "string",
								desc: "Unique identifier for the batch submission.",
							},
							{
								name: "total",
								type: "number",
								desc: "Total number of verification objects submitted.",
							},
							{
								name: "submitted",
								type: "number",
								desc: "Number of verifications successfully processed.",
							},
							{
								name: "failed",
								type: "number",
								desc: "Number of verifications that encountered errors.",
							},
							{
								name: "results",
								type: "array",
								desc: "Array of individual verification results, each with index, id, verification_type, status, and response_data.",
							},
							{
								name: "results[].index",
								type: "number",
								desc: "Position of this result corresponding to the input array index.",
							},
							{
								name: "results[].id",
								type: "string",
								desc: "Unique verification ID for this individual record.",
							},
							{
								name: "results[].status",
								type: "string",
								desc: "Status of the verification: verified, not_found, failed, or error.",
							},
							{
								name: "results[].response_data",
								type: "object | null",
								desc: "Parsed identity data if verified; null if not found or failed.",
							},
						].map((f) => (
							<div
								key={f.name}
						...
						<table className="w-full">
							...
							<tbody className="divide-y divide-gray-200">
								{BULK_RESPONSE_CODES.map((r) => (
									<tr
										key={r.code}
										className={r.isError ? "bg-red-50" : ""}
									>
										<td className="px-4 py-3">
											<code
												className={`text-sm font-mono font-semibold ${r.code === "200" || r.code === "207" ? "text-green-600" : r.isError ? "text-red-600" : "text-gray-700"}`}
											>
												{r.code}
											</code>
										</td>
					...
					<ul className="space-y-2 text-sm text-gray-600">
						...
						<li className="flex items-start gap-2">
							...
							<span>
								...
								field in results to map responses back to your input records
								reliably.
							</span>
						</li>
						<li className="flex items-start gap-2">
							...
							<span>
								...
								count first; handle partial failures gracefully using HTTP 207.
							</span>
						</li>
						<li className="flex items-start gap-2">
							<CheckIcon className="text-teal-500 mt-0.5 flex-shrink-0 w-4 h-4 flex items-center justify-center" />
							<span>
								...
								to validate your bulk payload structure before going live.
							</span>
						</li>
						<li className="flex items-start gap-2">
							<CheckIcon className="text-teal-500 mt-0.5 flex-shrink-0 w-4 h-4 flex items-center justify-center" />
							...
							<tbody className="divide-y divide-gray-200">
								{RESPONSE_CODES.map((r) => (
									...
									>
										<td className="px-4 py-3">
											<code
												className={`text-sm font-mono font-semibold ${r.code === "200" ? "text-green-600" : r.isError ? "text-red-600" : "text-gray-700"}`}
											>
												{r.code}
											</code>
										</td>
